package com.netelsan.ipinterkompanel.multi_call.sound_stream;
...
public class MediaStreamClient {
    ...
    public MediaStreamClient(final Context ctx, final String ip, final int port) {
        ...
        audioTrack = new AudioTrack(AppRTCAudioManager.SELECTED_SOUND_CHANNEL, frequency, channelConfiguration, audioEncoding, playBufSize, AudioTrack.MODE_STREAM);
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    e.printStackTrace();
                    Intent intent = new Intent().setAction("tw.rascov.MediaStreamer.ERROR").putExtra("msg", e.toString());
