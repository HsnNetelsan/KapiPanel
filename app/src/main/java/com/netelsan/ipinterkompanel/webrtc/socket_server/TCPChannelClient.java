...
package com.netelsan.ipinterkompanel.webrtc.socket_server;
...
public class TCPChannelClient {
    ...
    public TCPChannelClient(ExecutorService executor, TCPChannelEvents eventListener, String ip, int port) {
        ...
        Log.d(Constants.LOG_TAG, "1907 selfIP=" + selfIP + " connectingIP=" + ip);
        if(selfIP.equals(ip)) {// sunucu olarak çalışacak
            socket = new TCPSocketServer(address, port);// burası arayan taraftan aktif olacak
            socket.start();
        } else {
            //burada ufak bir delay iyi olabilir çünkü o sırada server devreye giremeyebiliyor.
            // ilk deneme olarak 500 ms yapılıyor.
            // 2500 ms ideal gibi duruyor

            Timer timer = new Timer();
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    startClientSocketTask(address, port);
                }
            }, 2500);
    ...
    private void startClientSocketTask(InetAddress address, int port) {
        new ClientSocketTask(address, port).executeOnExecutor(Constants.DEFAULT_EXECUTOR);
    }


    class ClientSocketTask extends AsyncTask<Void, Void, Void> {
        InetAddress address;
        int port;

        public ClientSocketTask(InetAddress address, int port) {
        ...
        protected Void doInBackground(Void... voids) {
            socket = new TCPSocketClient(address, port);// burası karşı taraftan arayanda aktif olacak
            return null;
        }
    ...
    public abstract class TCPSocket extends Thread {
        ...
        public void run() {
            ...
            synchronized(rawSocketLock) {
                ...
                try {
                    out = new PrintWriter(new OutputStreamWriter(rawSocket.getOutputStream(), Charset.forName("UTF-8")), true);
                    in = new BufferedReader(new InputStreamReader(rawSocket.getInputStream(), Charset.forName("UTF-8")));
                } catch(IOException e) {
